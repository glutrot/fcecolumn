 ...
 */
class tx_fcecolumn_newContentElement_hook implements \TYPO3\CMS\Backend\Wizard\NewContentElementWizardHookInterface {
  ...
  public function manipulateWizardItems(&$wizardItems, &$parentObject) {
    ...
    // get tv-field
    $colpos = '';
	  $colAr = \TYPO3\CMS\Core\Utility\GeneralUtility::trimExplode(':',$parentObject->parentRecord,1);
    ...
    // remove fce
    // get remove
    $TCEFORM_TSconfig = \TYPO3\CMS\Backend\Utility\BackendUtility::getTCEFORM_TSconfig('tx_fcecolumn', array('pid' => $parentObject->id));
    $tceItems = array();
    if ($colpos != '' && is_array($TCEFORM_TSconfig[$colpos])) {
      foreach ($TCEFORM_TSconfig[$colpos] as $ds => $dsv) {      
        ...
        if ($dsv['onlyItems']) {
            $removePattern = '*';
            $onlyItems = \TYPO3\CMS\Core\Utility\GeneralUtility::trimExplode(',',$dsv['onlyItems'],1);
        }
        ...
        if ($removePattern == '*'){
            ...
            $removeItems = \TYPO3\CMS\Core\Utility\GeneralUtility::trimExplode(',',$removePattern,1);
    ...
    $TCEFORM_TSconfig = \TYPO3\CMS\Backend\Utility\BackendUtility::getTCEFORM_TSconfig('tt_content', array('pid' => $parentObject->id));
    if ($colpos != '') {
      ...
      if (strlen($addvals)>0) {
          if ($onlyTypesItems){
             $onlyTypesArray = \TYPO3\CMS\Core\Utility\GeneralUtility::trimExplode(',', $onlyTypesItems,1);
          }
    ...
    $onlyPlugins = \TYPO3\CMS\Core\Utility\GeneralUtility::trimExplode(',', $onlyPluginsPattern, 1);
    ...
    //    debug($removePlugins);
        ...
        $explodedPluginList = \TYPO3\CMS\Core\Utility\GeneralUtility::trimExplode(',', $removePluginsPattern,1);
